...
function stat(o, prefix = '') {
  ...
  if(enumerate.includes(prefix)) {
    t = stats[prefix + '"' + o + '"'] || (stats[prefix + '"' + o +'"'] = {has: 0, count: 0, samples: []})
    t.has++; 
    t.count++; 
  }
  ...
  s.has++;
  if(typeof o !== 'object' && s.samples.length < 10 && (s.samples.length < 1 || Math.random() < 0.03)) {
    s.samples.push(o);
  }
  if(Array.isArray(o)) {
    for(const x of o) {
...
function writeStats() {
  ...
  for(const k of keys) {
    let s = stats[k];
    let line = `${k} ${(1000 * s.has / s.count | 0)/10}% ${s.has}/${s.count} ${JSON.stringify(s.samples)}`;
